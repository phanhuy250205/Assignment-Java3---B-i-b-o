...
.news-card {
	...
	margin: 10px;
	min-height: 350px;
	/* Đảm bảo thẻ có chiều cao tối thiểu để các nút luôn căn đều */
}
...
<body>
	...
	<div class="container">
		...
		<form
			action="${pageContext.request.contextPath}/news/${empty news.id ? 'insert' : 'update'}"
			method="post" enctype="multipart/form-data">
			<input type="hidden" id="newsId" name="id"
				value="${not empty news.id ? news.id : ''}"> <label
				for="newsTitleInput">Tiêu đề:</label> <input type="text"
				id="newsTitleInput" name="title"
				value="${not empty news.title ? news.title : ''}"
				placeholder="Tiêu đề tin tức" required> <label
				for="newsContent">Nội dung:</label>
			<textarea id="newsContent" name="content"
				placeholder="Nội dung tin tức" required>${not empty news.content ? news.content : ''}</textarea>

			<label for="newsFile">Hình ảnh:</label> <input type="file"
				id="newsFile" name="image" accept="image/*"> <label
				for="postedDate">Ngày đăng:</label> <input type="date"
				id="postedDate" name="postedDate"
				value="${not empty news.postedDate ? news.postedDate : ''}" required>

			<label for="newsAuthor">Tác giả:</label> <input type="text"
				id="newsAuthor" name="author"
				value="${not empty news.author ? news.author : ''}"
				placeholder="Tác giả" required> <label for="viewCount">Lượt
				xem:</label> <input type="number" id="viewCount" name="viewCount"
				value="${not empty news.viewCount ? news.viewCount : ''}"
				placeholder="Lượt xem" required> <label for="categoryId">Mã
				loại tin:</label> <select name="categoryId">
				<c:forEach items="${categories}" var="category">
					<option value="${category.id}">${category.name}</option>
				</c:forEach>
			</select> <label for="home">Xuất hiện trên trang chủ:</label> <select
				id="home" name="home" required>
			...
			<c:if test="${not empty newsList}">
				<c:forEach var="news" items="${newsList}">
					<div class="news-card">
						...
						<p>
							<strong class="content">Nội dung:</strong> ${news.content}
						</p>
						<p>
						...
						<p>
							<strong>Mã loại tin:</strong> ${news.categoryId}
						</p>
						<p>
							<strong>Xuất hiện trên trang chủ:</strong> ${news.home ? 'Có' : 'Không'}
						</p>

						<div class="actions">
							<form action="${pageContext.request.contextPath}/news/edit"
								method="get">
