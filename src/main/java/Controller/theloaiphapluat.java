...
public class theloaiphapluat extends HttpServlet {
    private static final long serialVersionUID = 1L;

    private Newsdao newsdao;

    // Khởi tạo đối tượng Newsdao
    public theloaiphapluat() {
        this.newsdao = new Newsdao();
    }

    // Phương thức xử lý GET request
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        String categoryId = "C03 ";  // ID của danh mục
        ...
        request.setAttribute("cultureNew", cultureNews);

        // Chuyển tiếp request tới JSP để hiển thị dữ liệu
        request.getRequestDispatcher("/views/TheLoai/loai-phap-luat.jsp").forward(request, response);
    }
