...
public class theloaivanhoa extends HttpServlet {
    private static final long serialVersionUID = 1L;

    private Newsdao newsdao;

    // Khởi tạo đối tượng Newsdao
    public theloaivanhoa() {
        this.newsdao = new Newsdao();
    }

    // Phương thức xử lý GET request
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        String categoryId = "C02";  // ID của danh mục
        ...
        request.setAttribute("vanhoanews", cultureNews);

        // Chuyển tiếp request tới JSP để hiển thị dữ liệu
        request.getRequestDispatcher("/views/TheLoai/loai-van-hoa.jsp").forward(request, response);
    }
